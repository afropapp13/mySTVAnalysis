		...
		for (int WhichPlot = 0; WhichPlot < N1DPlots; WhichPlot ++) {		
			...
			if (string(PlotNames[WhichPlot]).find("Serial") != std::string::npos) {	
				...
				for (int ipoint = 0; ipoint < nbreaks + 1; ipoint ++) {
					...
					if (ipoint == 0) { slice.at(ipoint)->DrawLatex( bin_break_points.at(ipoint) / 5. , 0.6 * 89., LatexLabel[phase_space ]); }
					else { slice.at(ipoint)->DrawLatex( bin_break_points.at(ipoint - 1) + ( bin_break_points.at(ipoint) - bin_break_points.at(ipoint-1) ) / 5. , 0.6 * 89., LatexLabel[phase_space ]); }
