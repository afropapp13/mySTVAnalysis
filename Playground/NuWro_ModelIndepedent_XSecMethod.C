...
double GetSpread(std::vector<double> BinContent) {
	...
	double spread = TMath::Abs(max-min) / TMath::Sqrt(2); // sqrt(12) = std of uniform distribution
//	double spread = TMath::Abs(max-min) / TMath::Sqrt(12); // sqrt(12) = std of uniform distribution
...
void NuWro_ModelIndepedent_XSecMethod() {
...
//	PlotNames.push_back("DeltaPTPlot");
	...
	TFile* fUnc = TFile::Open(PathToFiles+UBCodeVersion+"/NuWroWienerSVD_UnfoldingUnc_Combined_"+UBCodeVersion+".root","recreate");		
	...
	for (int WhichPlot = 0; WhichPlot < NPlots; WhichPlot++) {
		...
		TString CanvasName = "NuWro_ModelIndependent_Comparison_"+PlotNames[WhichPlot];
		...
		TLegend* leg = new TLegend(0.1,0.89,0.95,0.98);
		leg->SetNColumns(2);		
...
//			TruePlots[WhichPlot][WhichFile]->SetLineColor(Colors[WhichFile]);
			TruePlots[WhichPlot][WhichFile]->SetLineColor(kRed+2);
...
//			Plots[WhichPlot][WhichFile]->Draw("e1x0 same");
			if (WhichFile == 0) { 
				...
				// To obtain the uncertainty as the difference between the NuWro Data and the NUWro MC 

				TH1D* DataClone = (TH1D*)(Plots[WhichPlot][WhichFile]->Clone());

				for (int WhichBin = 0; WhichBin < n; WhichBin++) {

					double DataEntry = Plots[WhichPlot][WhichFile]->GetBinContent(WhichBin+1);
					double MCEntry = TruePlots[WhichPlot][WhichFile]->GetBinContent(WhichBin+1);					
					double DataMCUncEntry = TMath::Abs(DataEntry - MCEntry)/TMath::Sqrt(12);

					DataClone->SetBinContent(WhichBin+1,DataMCUncEntry);

				}

				fUnc->cd();		
				DataClone->Write("NuWroDataMCUnfUnc_"+PlotNames[WhichPlot]);
				TruePlots[WhichPlot][WhichFile]->Write("AltTrue"+PlotNames[WhichPlot]);

				leg->AddEntry(TruePlots[WhichPlot][WhichFile],"NuWro Truth","l");				
				
			}
			else { Plots[WhichPlot][WhichFile]->Draw("p0 hist same"); }			
			leg->AddEntry(Plots[WhichPlot][WhichFile],"NuWro Data " +Label[WhichFile],"p");		

			//------------------------------------//

			// Assigning the nuwro uncertainty as the data point spread / sqrt(12)
			...
			fUnc->cd();		
			Uncertainty->Write("NuWroDataUnfUnc_"+PlotNames[WhichPlot]);
