...
void StoreCanvas(TH2D* h, TString Label, TString Syst, TString PlotNames, TString BaseMC, TString Runs, TString Tune) {
	...
	PlotCanvas->SaveAs(plot_path+BaseMC+"/ER_"+Tune+"WienerSVD_"+Syst+"_"+Label+"CovarianceMatrices_"+PlotNames+BaseMC+"_"+Runs+".pdf");
...
void mcc9_10_event_rate_covariances(TString Syst = "None",TString BaseMC = "mcc9_10_Overlay9",TString BeamOnSample = "mcc9_10_BeamOn9",TString BeamOffSample = "mcc9_10_ExtBNB9",TString DirtSample = "mcc9_10_OverlayDirt9", TString Tune = "") {
...
//	PlotNames.push_back("DeltaAlphaTPlot"); 
	...
	if (Syst == "XSec") {
		...
		UniAltModels.push_back("_AxFFCCQEshape_UBGenie"); Universes.push_back(2); // check
	...
	for (int WhichRun = 0; WhichRun < NRuns; WhichRun++) {
		...
		TString FileName = migration_matrices_path+Tune+"ER_WienerSVD_"+Syst+"_CovarianceMatrices_"+BaseMC+"_"+xsec_Runs[WhichRun]+".root";
		if (BeamOnSample != "mcc9_10_BeamOn9") { FileName = migration_matrices_path+BeamOnSample+"ER_WienerSVD_"+Syst+"_CovarianceMatrices_"+BaseMC+"_"+xsec_Runs[WhichRun]+".root"; }
		...
		for (int WhichPlot = 0; WhichPlot < N1DPlots; WhichPlot ++) {
			...
			for (int alt = 0; alt < NAltModels; alt++ ) {
				...
				if (Syst == "NuWro") { TStringAltBaseMC = ExactFileLocation+"/"+Tune+"ncpi0_mcc9_10_Overlay9NuWro_"+xsec_Runs[WhichRun]+CutExtension+".root"; }
				...
				if (Syst == "NuWro") { TStringAltBaseMCResponseMatrix = migration_matrices_path+Tune+"FileResponseMatrices_mcc9_10_Overlay9NuWro_"+xsec_Runs[WhichRun]+".root";}
			...
			if (BeamOnSample == "mcc9_10_BeamOn9" || BeamOnSample == "mcc9_10_Overlay9") { // Beam On Sample, need to subtract ALL backgrounds
				...
				for (int alt = 0; alt < NAltModels; alt++ ) {
					...
					AltBeamOnPlots[WhichPlot][alt] = (TH1D*)AltNCCOHPlots[WhichPlot][alt]->Clone();
			...
			if (	BaseMC == "mcc9_10_Overlay9" 
				...
				TString EventRateCanvasName = "/ER_"+Tune+"EventRate_WienerSVD_"+Syst+"_CovarianceMatrices_"+PlotNames[WhichPlot]+BaseMC+"_"+xsec_Runs[WhichRun]+".pdf";
			...
			if (
				BaseMC == "mcc9_10_Overlay9" && (Syst == "XSec" || Syst == "DetailedXSec" || Syst == "G4" || Syst == "Flux" || Syst == "NuWro" || 
				...
				for (int unialt = 0; unialt < (int)(UniAltModels.size()); unialt++ ) {
					...
					TString EventRateCanvasName = "/ER_"+Tune+"EventRate_WienerSVD_"+Syst+UniAltModels[unialt]+"_CovarianceMatrices_"+PlotNames[WhichPlot]+BaseMC+"_"+xsec_Runs[WhichRun]+".pdf";
					EventRatePlotCanvas->SaveAs(plot_path+BaseMC+EventRateCanvasName);
					delete EventRatePlotCanvas;
