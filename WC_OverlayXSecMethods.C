...
//	FileNames.push_back("Overlay9_Run3"); Label.push_back("Run3"); 
	...
	for (int WhichFile = 0; WhichFile < NFiles; WhichFile++) {
		...
		for (int WhichPlot = 0; WhichPlot < NPlots; WhichPlot++) {
			...
			TString CanvasName = "WC_Comparison_"+PlotNames[WhichPlot]+"_"+FileNames[WhichFile];
			...
			TFile* f = TFile::Open(PathToFiles+UBCodeVersion+"/WC_ExtractedXSec_"+FileNames[WhichFile]+"_"+UBCodeVersion+".root","readonly");

			TH1D* Plots = (TH1D*)(f->Get("unf"));
			TH2D* Cov = (TH2D*)(f->Get("unfcov"));			
			...
			for (int WhichBin = 1; WhichBin <= n; WhichBin++ ) {
				...
				Plots->SetBinContent(WhichBin,Plots->GetBinContent(WhichBin)/Plots->GetBinWidth(WhichBin));
				Plots->SetBinError(WhichBin,TMath::Sqrt( Cov->GetBinContent(WhichBin,WhichBin) )/Plots->GetBinWidth(WhichBin));
			...
			leg->AddEntry(Plots,"WC","p");
			leg->AddEntry(Offset,"Me","p");
