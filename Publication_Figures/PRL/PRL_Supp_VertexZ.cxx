...
//	Runs.push_back("Run5");
	...
	for (int irun = 0; irun < NRuns; irun++) {
		...
		TString TxtName = "/home/afroditi/Dropbox/Apps/Overleaf/MicroBooNE_KinematicImbalance/XSec/XSec_VertexZ.txt";
		ofstream myTxtFile;
		...
		for (int iplot = 0; iplot < NPlots; iplot ++) {
			...
			TString CanvasName = "VertexZ_" + PlotNames[iplot]+"_"+Runs[irun];
			...
			TLegend* leg = new TLegend(0.44,0.4,0.86,0.5);
			leg->SetBorderSize(0);
			...
			TLegend* legData = new TLegend(0.43,0.3,0.85,0.4);
			...
			BeamOnShapeStat->GetYaxis()->SetTitle(VarLabel[ Mapping[PlotNames[iplot]] ]);
			BeamOnShapeStat->GetYaxis()->SetTitleOffset(1.5);

			BeamOnShapeStat->GetXaxis()->SetTitle("Vertex z [cm]");
			...
			myTxtFile << "Cross Section \\vertex z " << LatexLabel[ Mapping[ PlotNames[iplot] ] ] << endl;
			...
			myTxtFile << "Unfolded Covariance Matrix \\vertex z " << LatexLabel[ Mapping[ PlotNames[iplot] ] ] << endl;
			myTxtFile << "       ";
			...
			myTxtFile << "Additional Smearing Matrix \\vertex z " << LatexLabel[ Mapping[ PlotNames[iplot] ] ] << endl;
			myTxtFile << "       ";
			for (int ybin = 1; ybin <= NBins; ybin++) { myTxtFile << "Bin " << ybin << "  "; }
			myTxtFile << endl;
			...
			for (int igen = 0; igen < NMC; igen++) {			
				...
				TLegendEntry* lGenie = leg->AddEntry(MCPlot[igen],Label[igen],"l");				
			...
			PlotCanvas->SaveAs("/home/afroditi/Dropbox/Apps/Overleaf/MicroBooNE_KinematicImbalance/Figures/PRL_Supp_"+PlotNames[iplot]+"_"+Runs[irun]+"_"+UBCodeVersion+".pdf");
