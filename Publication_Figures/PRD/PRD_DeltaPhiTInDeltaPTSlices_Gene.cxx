...
void PRD_DeltaPhiTInDeltaPTSlices_Gene() {
	...
	PlotNames.push_back("DeltaPhiTPlot"); PanelLabels.push_back("(a)"); Min.push_back(0.); Max.push_back(0.36); Units.push_back("[$10^{-38}\\frac{cm^{2}}{deg\\,^{40}Ar}$]"); SaveFig.push_back("Fig76");
	PlotNames.push_back("SerialDeltaPhiT_DeltaPTPlot_0"); PanelLabels.push_back("(b)"); Min.push_back(0.); Max.push_back(1.6); Units.push_back("[$10^{-38}\\frac{cm^{2}}{deg\\,(GeV/\\textit{c})\\,^{40}Ar}$]"); SaveFig.push_back("Fig77");
	PlotNames.push_back("SerialDeltaPhiT_DeltaPTPlot_1");PanelLabels.push_back("(c)"); Min.push_back(0.); Max.push_back(0.34); Units.push_back("[$10^{-38}\\frac{cm^{2}}{deg\\,(GeV/\\textit{c})\\,^{40}Ar}$]"); SaveFig.push_back("Fig78");	
	PlotNames.push_back("SerialDeltaPhiT_DeltaPTPlot_2"); PanelLabels.push_back("(d)"); Min.push_back(0.); Max.push_back(0.044); Units.push_back("[$10^{-38}\\frac{cm^{2}}{deg\\,(GeV/\\textit{c})\\,^{40}Ar}$]"); SaveFig.push_back("Fig79");
...
//	Runs.push_back("Run5");
	...
	for (int irun = 0; irun < NRuns; irun++) {
		...
		TString TxtName = "/home/afroditi/Dropbox/Apps/Overleaf/MicroBooNE_KinematicImbalance_PRD_Rename/PRD_XSec_DeltaPhiTInDeltaPT.tex";
		ofstream myTxtFile;
...
//			TLegend* legData = new TLegend(0.51,0.68,0.97,0.8);
			TLegend* legData = new TLegend(0.5,0.77,0.92,0.89);
			if (iplot == 3) { legData = new TLegend(0.24,0.76,0.66,0.88); }			
			...
			TLegend* legUnc = new TLegend(0.492,0.71,0.892,0.77);
			if (iplot == 3) { legUnc = new TLegend(0.232,0.7,0.622,0.76); }						
...
//			TLegend* leg = new TLegend(0.52,0.81,0.98,0.9);
			TLegend* leg = new TLegend(0.5,0.51,0.94,0.71);	
			if (iplot == 3) { leg = new TLegend(0.64,0.7,0.94,0.88); }					
			leg->SetBorderSize(0);
			...
			myTxtFile << "\\multicolumn{5}{|c|}{Cross Section $\\delta\\phi_{T}$, " << LatexLabelString << "} \\\\" << endl;
			myTxtFile << "\\hline" << endl;
			myTxtFile << "\\hline" << endl;			
			...
			// myTxtFile << "\\hline" << endl;						
			// myTxtFile << "\\multicolumn{" << NBins+1 << "}{|c|}{Unfolded Covariance Matrix $\\delta\\phi_{T}$, " << LatexLabelString << "} \\\\" << endl;
			// myTxtFile << "\\hline" << endl;
			// myTxtFile << "\\hline" << endl;
